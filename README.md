...
| Offsite backup sync | ✅ Verified for real | `hub:backup-database` uploads to an S3-compatible disk (AWS S3 / DO Spaces / B2 / MinIO — set `BACKUP_S3_*`), skipped cleanly when unconfigured. Verified against a real MinIO container: uploaded, re-downloaded, MD5 identical to the local file (no corruption). See DEPLOYMENT.md §9a |
